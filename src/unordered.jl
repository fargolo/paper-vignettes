...
function prob_occurrences_bin_constr(k,n,d;alpha=0.05)
    probs = Float64[]
    counts = BigInt[]
    total_count = 2^n
    # Test: Sum probabilities of p(k), k > k_observed
    for i in 1:n
        cur_count = bin_words_with_k_occurences_constr(i,n,d)
        cur_p = bin_words_with_k_occurences_constr(i,n,d)/ total_count
#       print(Float64(cur_p)) 
        push!(probs,cur_p)
        push!(counts,cur_count)
    end
