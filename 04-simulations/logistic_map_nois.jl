...
# White noise  
white_n_time_series = rand(Distributions.Normal(0,0.03),
                                total_time+1)
...
X2n, X4n, Xtn, Xcn = map(x -> white_n_time_series .+ [Matrix(x)...],  
                                [X2 , X4 , Xt , Xc])

a1 = Plots.plot([X2n...][1:200])
b1 = Plots.plot([X4n...][1:200])
c1 = Plots.plot([Xtn...][1:200])
d1 = Plots.plot([Xcn...][1:200])
traj_plot = plot(a1,b1,c1,d1, layout = (4, 1),legend=false,
        title=["r = 3.20" "r = 3.50" "r = 3.56" "r = 4.00"])



SymbolicInference.persistence_barcode(Xtn; 
        range = collect(0.1:0.1:0.9), n_windows=1, alpha_thresh=0.5)
